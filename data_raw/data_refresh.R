...
usethis::use_data(covid19sk, overwrite = TRUE)
write.csv(covid19sk, "csv/covid19sk.csv", row.names = FALSE)

#----------------Iran----------------
# Summarise table of cases in the Iran
# Using : https://en.wikipedia.org/wiki/COVID-19_pandemic_in_Iran

url_iran <-  "https://en.wikipedia.org/wiki/COVID-19_pandemic_in_Iran"

iran_raw <- url_iran %>%
  xml2::read_html() %>%
  rvest::html_node(xpath = '//*[@id="mw-content-text"]/div/div[17]/table') %>%
  rvest::html_table(fill = TRUE,
                    header = TRUE)
...
covid19iran <- iran_raw[-1, ] %>% stats::setNames(iran_names) %>%
...
usethis::use_data(covid19iran, overwrite = TRUE)

write.csv(covid19iran, "csv/covid19iran.csv", row.names = FALSE)
